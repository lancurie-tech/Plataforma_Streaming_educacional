...
| *Preview* de PR no mesmo *backend* de prod | *Previews* de Hosting e *build* no **projeto dev** (PR → Firebase Hosting *preview* no `medivox-sp-dev` ou ID equivalente) |
...
- **Duplicas:** projeto Firebase (novo projeto na consola), eventualmente **domínio** de preview ou subdomínio `staging.medivox...`.
...
2. Nome sugerido: `medivox-dev` ou `medivox-staging` (nome interno; o **Project ID** tem de ser único globalmente).
...
firebase login
firebase use --add
# Escolhe o projeto medivox-dev e dá um alias, ex.: dev
firebase use dev
...
{
  "projects": {
    "default": "medivox-prod",
    "dev": "medivox-dev-id-real"
  }
}
```
...
3. Atualiza os **GitHub Actions secrets** (e variáveis) de *build* de **produção** com o `firebaseConfig` do projeto **produção** Medivox.
4. Idem para os *secrets* `*_DEV` e *deploy* de `dev` / PRs com o projeto **dev** Medivox.
...
- [ ] Projeto Firebase `medivox-dev` criado
